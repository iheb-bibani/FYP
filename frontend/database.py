...
def get_optimal_weights(_connection: psycopg2.extensions.connection, run_id: int) -> list:
    cursor = _connection.cursor()
    cursor.execute(
        "SELECT weight, returns, volatility FROM optimal_weights WHERE run_id=%s", (run_id,)
    )
    rows = cursor.fetchall()
    cursor.close()
...
def get_ticker_data(_connection: psycopg2.extensions.connection, run_id: int) -> List[str]:
    cursor = _connection.cursor()
    cursor.execute(
        "SELECT tickers FROM ticker_run WHERE id=%s", (run_id,)
    )
    row = cursor.fetchone()
    cursor.close()
...
def get_run_id(_connection: psycopg2.extensions.connection, start_date: str, end_date: str) -> int:
    cursor = _connection.cursor()
    cursor.execute(
        "SELECT id FROM ticker_run WHERE start_date=%s AND end_date=%s", (start_date, end_date)
    )
