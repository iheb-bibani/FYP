...
def download_and_store_historical_data():
    ...
    for idx, row in data.iterrows():
        ...
        cursor.execute(f'CREATE TABLE IF NOT EXISTS {table_name} (Date TEXT, Open REAL, High REAL, Low REAL, Close REAL, Volume INTEGER, Dividends REAL, Stock_Splits REAL)')

        # Get the latest date in the database
        latest_date_in_db = cursor.execute(f'SELECT MAX(Date) FROM {table_name}').fetchone()[0]
        if latest_date_in_db:
            # Start from the day after the latest date in the database
            start_date = (datetime.datetime.strptime(latest_date_in_db, '%Y-%m-%d') + datetime.timedelta(days=1)).strftime('%Y-%m-%d')
        else:
        ...
        for index, row in historical_data.iterrows():
            cursor.execute(f"INSERT INTO {table_name} VALUES ('{index.strftime('%Y-%m-%d')}', {row['Open']}, {row['High']}, {row['Low']}, {row['Close']}, {row['Volume']}, {row['Dividends']}, {row['Stock Splits']})")
